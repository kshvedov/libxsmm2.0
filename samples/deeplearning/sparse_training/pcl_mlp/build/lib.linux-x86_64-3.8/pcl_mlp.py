...
class XsmmFC(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if XsmmFC.use_sparse_kernels:
            ...
            print("\n█*█*█ -> Using SPARSE kernels for backward pass")
            ...
            print("===== grad input ====")
            print(grad_input)
            #np.savetxt('grad_input.txt', grad_input.numpy())
            f = open("grad_input.txt", "w")
            torch.set_printoptions(profile="full")
            print(grad_input, file=f)
            f.close()

            f = open("2_input.txt", "w")
            print(input, file=f)
            torch.set_printoptions(profile="default")
            f.close()
            print("===== grad input ====")
            ...
            print("===== grad weight ====")
            print(grad_weight)
            #np.savetxt('grad_weight.txt', grad_weight.numpy())
            f = open("grad_weight.txt", "w")
            torch.set_printoptions(profile="full")
            print(grad_weight, file=f)
            f.close()

            f = open("3_input.txt", "w")
            print(input, file=f)
            f.close()

            f = open("grad_weight_weight.txt", "w")
            print(weight, file=f)
            f.close()

            f = open("grad_weight_output.txt", "w")
            print(grad_output, file=f)
            torch.set_printoptions(profile="default")
            f.close()
            print("===== grad weight ====")
